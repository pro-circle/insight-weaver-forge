...
export function AutomateDialog({
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select("automation_enabled, automation_time")
      .eq("id", userId)
      .single()
      .then(({ data }) => {
        if (data) {
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <span className="mt-1 block text-xs">
              ✅ Runs on the server (pg_cron + Supabase Edge Function) — no need to keep the app
              open.
            </span>
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Runs at the time below (your local time).
                </p>
              </div>
