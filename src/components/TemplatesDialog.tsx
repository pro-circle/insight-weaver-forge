...
import {
  DEFAULT_EMAIL_BODY, DEFAULT_EMAIL_SUBJECT, DEFAULT_SMS, DEFAULT_WHATSAPP, TEMPLATE_VARS,
} from "@/lib/templates";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit your email / WhatsApp / SMS templates. Keep the <code>{`{{variables}}`}</code> intact.
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <>
            <div className="mt-2 rounded-md border bg-secondary/40 p-3 text-xs">
              Available variables: {TEMPLATE_VARS.map((v) => <code key={v} className="mr-1">{`{{${v}}}`}</code>)}
            </div>
            ...
            <Tabs defaultValue="email" className="mt-3">
              ...
              <TabsContent value="email" className="space-y-3 pt-4">
                <div><Label htmlFor="template-email-subject">Subject</Label><Input id="template-email-subject" name="email_subject_template" value={f.email_subject_template} onChange={(e) => setF({ ...f, email_subject_template: e.target.value })} /></div>
                <div><Label htmlFor="template-email-body">Body</Label><Textarea id="template-email-body" name="email_body_template" rows={8} value={f.email_body_template} onChange={(e) => setF({ ...f, email_body_template: e.target.value })} /></div>
              </TabsContent>
              <TabsContent value="whatsapp" className="pt-4">
                <Label htmlFor="template-whatsapp-message">WhatsApp message</Label>
                <Textarea id="template-whatsapp-message" name="whatsapp_template" rows={4} value={f.whatsapp_template} onChange={(e) => setF({ ...f, whatsapp_template: e.target.value })} />
              </TabsContent>
              <TabsContent value="sms" className="pt-4">
                <Label htmlFor="template-sms-message">SMS message</Label>
                <Textarea id="template-sms-message" name="sms_template" rows={3} value={f.sms_template} onChange={(e) => setF({ ...f, sms_template: e.target.value })} />
              </TabsContent>
            </Tabs>
