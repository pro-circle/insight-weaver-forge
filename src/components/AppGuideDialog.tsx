...
export function AppGuideDialog() {
  ...
  useEffect(() => {
    if (open) logActivity("app_guide_opened", { view });
  }, [open, view]);
  useEffect(() => {
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {view === "smtp" &&
                "Complete step-by-step Gmail SMTP setup using a 16-character App Password."}
              {view === "workflow" &&
                "From importing customers to fully automated email reminders."}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-semibold group-hover:text-blue-900">
                    Email setup
                  </p>
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Import / add customers, set up daily auto-emails, and use semi-automated
                    WhatsApp & SMS.
                  </p>
                </button>
              </div>
          ...
          {view === "smtp" && (
            <ol className="mt-2 space-y-4 text-sm">
              ...
              <Step n={2} title="Generate a Gmail App Password (16 characters)">
                ...
                , pick app <b>Mail</b> and device <b>Other → "Invoice Flow"</b>, click{" "}
                <b>Generate</b>. Copy the 16-character password (spaces are fine, we strip them).
              </Step>
              <Step n={3} title="Open Settings → Gmail SMTP">
                ...
                <ul className="ml-4 mt-1 list-disc">
                  ...
                  <li>
                    App password: the 16-char password from step 2 (use the 👁 icon to verify)
                  </li>
                </ul>
                Click <b>Save</b>. Credentials are stored under your account with Row Level Security
                — only you can read them.
              </Step>
              <Step n={4} title="Send a test reminder">
                Add a customer with your own email, then click <b>Email</b> on their card. Check the
                inbox ✅. If you see a Gmail error, regenerate the App Password and re-paste.
              </Step>

              <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
                <b>Privacy:</b> SMTP credentials never leave your Supabase row. The browser never
                holds the password — it is read by the <code>send-email</code> Edge Function only at
                send time.
              </div>
            </ol>
          )}
          ...
          {view === "workflow" && (
            <ol className="mt-2 space-y-4 text-sm">
              <Step n={1} title="Add customers">
                ...
                (columns: name, phone, email, amount, status, due_date). Files are stored privately
                in <code>User_uploads</code>.
              </Step>
              <Step n={2} title="Customise message templates (optional)">
                Open <b>Templates</b> in the header. Edit email / WhatsApp / SMS wording. Keep{" "}
                <code>{`{{variables}}`}</code> intact — they're filled in per-customer. A UPI{" "}
                <b>Click here to pay now</b> link with the exact amount is auto-attached.
              </Step>
              <Step n={3} title="Set up automation (server-side cron)">
                Open <b>Automate email reminders</b>, toggle it on and pick a daily send time. A
                Supabase Edge Function (<code>automate-reminders</code>) is invoked by pg_cron,
                then sends email reminders for every customer whose status isn't <i>paid</i> and
                records clear send/skip/error details. <b>The app does not need to be open.</b>
              </Step>
              <Step n={4} title="Channels — what's automated vs. semi-automated">
                <div className="mt-2 grid gap-2 sm:grid-cols-3">
                  <Channel
                    ...
                    desc="Sent server-side via your Gmail SMTP."
                  />
                  <Channel
                    title="WhatsApp"
                    mode="Semi-automated"
                    desc="Opens a pre-filled wa.me chat — you tap Send."
                  />
                  <Channel
                    title="SMS"
                    mode="Semi-automated"
                    desc="Opens your phone's SMS app pre-filled — you tap Send."
                  />
                </div>
                <p className="mt-2 text-xs text-muted-foreground">
                  WhatsApp & SMS require the user's tap-to-send because mobile providers don't allow
                  silent sending from the browser.
                </p>
              </Step>
              <Step n={5} title="Track everything">
                Each send is logged to <code>notifications_sent</code> and{" "}
